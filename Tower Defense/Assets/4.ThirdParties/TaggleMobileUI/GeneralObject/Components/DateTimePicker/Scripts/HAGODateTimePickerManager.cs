...
public class HAGODateTimePickerManager : MonoBehaviour
{
    ...
    /// </summary>
	///<param name = "onDateSelectedEvent">Return a selected date after user selected day on calendar
	///<param name = "selectedDate">Default selected date when picker showed</param>
    ...
    /// </summary>
	///<param name = "onPeriodDateSelectedEvent">Return a selected date with period type selected on calendar
	///<param name = "dateMarkData">Dates will have circle mark icon</param>
	public void InitPeriodDatePicker(Action<HAGODatePickerPeriodType, DateTime, DateTime> onDateSelectedEvent)
	{
		HAGODateTimePickerControl.Api.ResultCallbackPeriodDateTimeEvent = onDateSelectedEvent;

        //init view
        HAGODateTimePickerView view = transform.Find("Canvas").GetComponent<HAGODateTimePickerView>();
        view.InitPeriodDatePicker();
	}
