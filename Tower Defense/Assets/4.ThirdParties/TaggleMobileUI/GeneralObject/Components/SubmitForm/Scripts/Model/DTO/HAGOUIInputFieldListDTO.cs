...
public class HAGOUIInputFieldListDTO
{
	...
	public HAGOUIInputFieldListDTO(
        string id, string title, string placeholder, List<string> content, string unit, HAGOUIInputFieldContentType contentType, string keyForm,
        float minValue = -1, float maxValue = -1, int minLength = -1, int maxLength = -1, bool isRequired = false
    )
	{
		ID = id;
		Title = title;
        Placeholder = placeholder;
        Contents = content ?? new List<string>();
        Unit = unit;
		KeyForm = keyForm;
    ...
    public HAGOUIInputFieldListDTO(JObject data)
	{
        ...
        Contents = data.Value<JArray>(HAGOServiceKey.PARAM_CONTENT).ToObject<List<string>>();
