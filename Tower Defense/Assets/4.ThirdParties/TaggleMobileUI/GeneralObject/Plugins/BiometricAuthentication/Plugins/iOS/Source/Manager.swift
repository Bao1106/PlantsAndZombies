...
final class Manager {
    ...
    private let helper = SecureEnclaveHelper(publicLabel: "com.taggle.biometric.publicKey", privateLabel: "com.taggle.biometric.privateKey", operationPrompt: "Authenticate to continue")
    ...
    func publicKey() throws -> String {
        ...
        return keys.public.pem // use pem
    }
    ...
    func sign(_ digest: Data) throws -> Data {
        ...
        let keys = try getKeys()
        let signed = helper.signCompat(privateKey: keys.private.underlying, rawData: digest)
        return signed ?? Data()
    }
    ...
    private func getKeys() throws -> (`public`: SecureEnclaveKeyData, `private`: SecureEnclaveKeyReference) {
        ...
        if let publicKeyRef = try? helper.getPublicKey(), let privateKey = try? helper.getPrivateKey() {
            ...
            if #available(iOS 11.3, *) {
                ...
                let accessControl = try helper.accessControlEarly(with: kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly)
                let keypairResult = try helper.generateKeyPair(accessControl: accessControl)
                          try helper.forceSavePublicKey(keypairResult.public)
            }
